...
const RocketLaunchScene = dynamic(
  () => import("@/components/rocket-launch/RocketLaunchScene"),
  { ssr: false },
);
  ...
  return (
    <div className="relative w-full h-[100dvh] overflow-hidden bg-black">
      ...
      <header className="absolute top-0 inset-x-0 z-30 flex items-start justify-between gap-2 px-3 pt-3 pb-2 sm:px-4 sm:pt-4 pointer-events-none">
        ...
        <div className="pointer-events-none flex flex-col items-center min-w-0 flex-1 px-1 sm:px-4 max-w-[min(100%,18rem)] sm:max-w-md mx-auto">
          <h1 className="text-xs sm:text-lg font-bold text-white glass-card px-3 py-1.5 sm:px-5 sm:py-2 rounded-xl border border-white/10 text-center leading-tight w-full truncate">
            Satellite Launch Lab
          </h1>
          <span className="mt-1.5 hidden min-[400px]:inline text-[9px] sm:text-[10px] font-semibold uppercase tracking-wider text-orange-400/90 glass-card px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full border border-orange-500/20">
            Interactive 3D · Rocketry
          </span>
        </div>
